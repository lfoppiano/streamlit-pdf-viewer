...
def test_page_separators_are_visible(page: Page):
    """
    Assert that PDF page separators are visible when enabled.
    
    Checks the Streamlit PDF viewer iframe for page divs and verifies the first page's computed `margin-bottom` is non-zero (indicating a separator between pages). Raises an AssertionError if no page is visible or if the computed `marginBottom` equals `'0px'`.
    """
    iframe_frame = page.frame_locator('iframe[title="streamlit_pdf_viewer.streamlit_pdf_viewer"]').nth(0)
